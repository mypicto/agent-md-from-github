...
from scripts.src.presentation.cli_controller import CLIController, parse_date
...
class TestCLIController:
    ...
    def test_run_正常実行_レビューコメントが収集される(self):
        ...
        # Mock dependencies
        with patch('scripts.src.presentation.cli_controller.ServiceFactory') as mock_factory:
    ...
    def test__get_github_token_キーリングから取得_トークンが返される(self):
        ...
        with patch('scripts.src.presentation.cli_controller.TokenManager') as mock_manager:
    ...
    def test_run_auth_オプションなし_ヘルプが表示される(self):
        ...
        with patch('scripts.src.presentation.cli_controller.argparse.ArgumentParser.parse_args') as mock_parse:
    ...
    def test__create_date_range_有効な日付範囲_DateRangeが作成される(self):
        ...
        with patch('scripts.src.presentation.cli_controller.TimezoneConverter') as mock_converter:
